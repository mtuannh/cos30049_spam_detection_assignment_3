...
const header = "text,label,spam_probability\n";
const lines = rows.map((r) => {
    const safeText = `"${(r.text || "").replace(/"/g, '""')}"`;
    const label = r.label ? "spam" : "ham";
    const prob = ((r.probability ?? 0) * 100).toFixed(2);
    return `${safeText},${label},${prob}`;
});
const blob = new Blob([header + lines.join("\n")], { type: "text/csv;charset=utf-8;" });
const url = URL.createObjectURL(blob);
const link = document.createElement("a");
link.href = url;
link.download = `batch_predictions_${Date.now()}.csv`;
